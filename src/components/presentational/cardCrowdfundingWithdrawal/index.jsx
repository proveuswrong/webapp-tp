...
export default function CardCrowdfundingWithdrawal(props) {
  console.log(props);
  return (
    <div className={`${styles.cardCrowdfundingWithdrawal}`}>
      <RewardSymbol />
      {props.contributed && props.withdrew && <span>You already withdrew.</span>}
      {props.contributed && !props.withdrew && (
        <div className={styles.totalWithdrawable}>
          Total Withdrawable Amount <EtherValue value={props.amount || 0} />
        </div>
      )}
      {!props.contributed && <span>You did not contribute the crowdfunding of the last dispute.</span>}

      <hr />
      <CustomButton modifiers="small" disabled={!props.contributed || withdrew} onClick={props.handleWithdrawRewards}>
